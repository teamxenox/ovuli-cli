...
const calculateOvuli = (lastPeriodDate, averageCycleDay) => {
  ...
  result["nextPeriodDate"]["day"] = moment(lastPeriodDate)
    .add(averageCycleDay - 1, "days")
    .format("DD");
  result["nextPeriodDate"]["month"] = moment(lastPeriodDate)
    .add(averageCycleDay - 1, "days")
    .format("MM");

  // To calculate next pregnancy date
  result["nextPregnancTestDate"]["day"] = moment(lastPeriodDate)
    .add(averageCycleDay, "days")
    .format("DD");
  result["nextPregnancTestDate"]["month"] = moment(lastPeriodDate)
    .add(averageCycleDay, "days")
    .format("MM");
  ...
  result["approximateOvulationDate"]["day"] = approximateOvulationDate.format(
    "DD"
  );
  result["approximateOvulationDate"]["month"] = approximateOvulationDate.format(
    "MM"
  );
